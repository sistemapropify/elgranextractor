...
def test_heatmap_page():
    ...
    try:
        ...
        # Mostrar resultados
        print(f"✅ Página cargada correctamente (HTTP {response.status})")
        print(f"✅ Script de Google Maps: {parser.has_google_maps_script}")
        print(f"✅ Script heatmap.js: {parser.has_heatmap_js}")
        print(f"✅ Div del mapa (heatmapMap): {parser.has_map_div}")
        print(f"✅ Clave API en URL: {parser.has_api_key}")
        ...
        if not parser.has_google_maps_script:
            print("❌ ERROR: No se encontró el script de Google Maps")
            return False
            
        if not parser.has_heatmap_js:
            print("❌ ERROR: No se encontró el script heatmap.js")
            return False
            
        if not parser.has_map_div:
            print("❌ ERROR: No se encontró el div del mapa (heatmapMap)")
            return False
        ...
        if not parser.has_api_key:
            print("❌ ADVERTENCIA: No se detectó clave API en el script de Google Maps")
            
        print("\n" + "=" * 60)
        print("✅ La página parece estar correctamente estructurada")
        print("\nRecomendaciones:")
        ...
        print(f"❌ Error al cargar la página: {type(e).__name__}: {e}")
        return False
