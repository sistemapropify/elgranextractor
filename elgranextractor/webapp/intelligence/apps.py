...
class IntelligenceConfig(AppConfig):
    ...
    def ready(self):
        ...
        # 4. Pre-calcular embeddings de templates del Semantic Router (F1-001)
        try:
            from .services.semantic_router import precompute_router_embeddings
            n_templates = precompute_router_embeddings()
            if n_templates > 0:
                logger.info(
                    f"Semantic Router: {n_templates} templates embeddeados "
                    f"para routing semántico de skills"
                )
        except Exception as e:
            logger.warning(
                f"No se pudieron pre-calcular embeddings del Semantic Router: {e}"
            )
