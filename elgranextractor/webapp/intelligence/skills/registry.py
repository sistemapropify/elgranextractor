...
class SkillRegistry:
    ...
    def find_best_skill(
        ...
        Estrategia de matching (F1-001):
        1. PRIMARIO: SemanticSkillRouter con embeddings E5-large + similitud coseno
           contra templates few-shot. Detecta semántica real, no solo keywords.
        2. FALLBACK: Keyword matching existente si el router no puede generar
           embedding o si el score está justo por debajo del threshold.
        ...
        # Logging estructurado de la decisión
        logger.info(
            f"[SkillRegistry] Routing decision — "
            ...
            f"fallback={routing_result.fallback_used}, "
            f"latency={routing_result.latency_ms:.1f}ms"
        )

        # Si el router aceptó, buscar la skill por nombre
        if routing_result.accepted and routing_result.skill_name:
            ...
            if skill and skill.can_user_access(user_level):
                logger.debug(
                    f"[SkillRegistry] Skill seleccionada por router semántico: "
                    f"'{skill.name}' (score: {routing_result.score:.4f})"
                )
                return skill
            elif skill:
                logger.debug(
                    f"[SkillRegistry] Skill '{routing_result.skill_name}' detectada "
                    f"pero usuario no tiene acceso (level={user_level})"
                )
            else:
                logger.warning(
                    f"[SkillRegistry] Skill '{routing_result.skill_name}' detectada "
                    f"por router pero no está registrada en el registry"
                )

        # ── 2. FALLBACK: Keyword matching existente ──
        # Solo si el router no encontró skill o no superó threshold
        intent_lower = intent.lower().strip()

        # ── Detección de distritos multi-palabra ──
        ...
        if best_score >= self.MIN_CONFIDENCE_THRESHOLD:
            logger.debug(
                f"[SkillRegistry] Skill seleccionada por keyword fallback: "
                f"'{best_skill.name}' (score: {best_score:.2f})"
            )
            return best_skill

        logger.debug(
            f"[SkillRegistry] Ninguna skill superó umbral "
            f"(mejor score keyword: {best_score:.2f}, "
