"""Remax: extractor específico con recorrido y persistencia compartidos."""
...
def _ejecutar_scraping(max_paginas=0, start_page=1, source_url=None, url=None,
    ...
    return run_paged('remax', max_paginas=max_paginas, start_page=start_page,
        source_url=requested_url('remax', {'source_url': source_url or url}),
...
class ScraperRemaxSkill(BaseSkill):
    name = 'scraper_remax'
    description = 'Extrae Remax desde la búsqueda configurada, con cobertura y recuperación verificables.'
    category = 'custom'
    access_level = 1
    is_active = True
    parameters_schema = {
    ...
    def validate_params(self, params):
        try:
            requested_url('remax', params)
            return int(params.get('max_paginas') or 0) >= 0
        except (ValueError, TypeError):
            return False

    def execute(self, params, context=None):
        return execute_paged_skill(self, 'remax', _ejecutar_scraping,
