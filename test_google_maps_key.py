...
try:
    ...
    print(f"✅ HTTP {response.status}: La clave parece válida (la API se carga)")
    ...
    if b'Google Maps JavaScript API' in content:
        print("✅ Contenido parece ser la API de Google Maps")
    else:
        print("⚠️  El contenido no parece ser la API de Google Maps")
